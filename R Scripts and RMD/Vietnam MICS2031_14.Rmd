...
merged_dat %<>%
  ...
  ungroup() %>% 
  mutate(
    no_missing_atten = no_missing_atten/hhs,
...
__For the above table the comments in STATA code are "tab no_missing_atten, miss //Values for 0 are less than 1% ",  if this is the case then there is some mistake I might have made in the above code chunck.__ 


```{r further newvars std mpi}
merged_dat %<>%
  group_by(hh_id) %>% 
  mutate(
...
__Section 2.2 needs to be checked before we can move forward.__
