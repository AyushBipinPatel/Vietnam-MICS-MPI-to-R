...
colnames(bh_dat) <-  str_to_lower(names(bh_dat))
...
bh_dat <- bh_dat %>% 
  mutate(
    ...
    child_died = ifelse(bh5 == 2,1,NA),
    child_died = ifelse(bh5 == 1,0,child_died),
    child_died = structure(
      ifelse(is.na(bh5), NA, child_died),
      labels = c("child is alive" = 0,
                 "child is dead" = 1)
    )
    ) %>% 
  group_by(ind_id) %>% 
  mutate(
    tot_child_died = sum(child_died, na.rm = T)
  ) %>% 
  ungroup() %>% 
  mutate(
    child18_died = child_died,
    child18_died = structure(
  ...
  mutate(
    tot_child18_died_5y = sum(
      child18_died[ydead_survey <= 5], na.rm = T)
  ) %>% 
  ungroup() %>% 
  mutate(
    ...
    tot_child18_died_5y = ifelse(
      child18_died == 1 & is.na(ydead_survey), 
...
Tables before droping duplicates

 Creating Tables for new variables check.

```{r before droptable age_death}
bh_dat %>% 
...
```{r before drop bh5 child_died, warning=FALSE}
bh_dat %>% 
  tabyl(bh5, child_died, show_na = T) 
```

```{r before drop tab child18_died}
bh_dat %>% 
...
```{r before drop tot_child_died tot_chil18_died_5y}
bh_dat %>% 
...
Again, Creating Tables for new variables check.
...
colnames(wm_dat) <- str_to_lower(names(wm_dat))
```
...
colnames(hh_dat) <- str_to_lower(names(hh_dat))
```
...
colnames(hl_dat) <- str_to_lower(names(hl_dat))
```
...
merged_dat %>% 
  mutate(
    fem_eligible = ifelse(!is.na(hl7), (hl7 > 0), NA), ## if hl7 is greater than 0, it cannot be NA, but I will stick to the STATA code for now
    ) %>% 
  group_by(hh_id) %>% 
  mutate(
    hh_n_fem_eligible = sum(fem_eligible, na.rm = T)
...
merged_dat %<>% 
  mutate(
    temp_f = ifelse((is.na(cm9a) & is.na(cm9b)),NA,
                    rowSums(.[,c(101,102)], na.rm = T)),
    temp_f = ifelse(
      (cm1 == 1 & cm8 ==2) | cm1 == 2,
      0, temp_f
    ),
...
merged_dat %<>%
  mutate(
    childu18_dies_per_wom_5y = ifelse((cm1 == 1&cm8==2)|cm1 == 2,                   0,childu18_dies_per_wom_5y),
    childu18_dies_per_wom_5y = ifelse(
      no_fem_eligible == 1,
  ...
  mutate(
    ...
    water_mdg = ifelse(
      (water_mdg == 1 & timetowater >= 30 & !is.na(timetowater) &
                                                     timetowater != 999 & timetowater != 998),
      0, water_mdg),
...
### Mean uncensored headcout check (without using sample weights)

```{r check mean uncensored}
...
merged_dat %>% 
  ...
  unlist() %>% 
  as.data.frame() %>% 
  kableExtra::kable(col.names = c("Mean Uncensred Headcount"))
```

### Mean uncensored headcout check (_with_ using sample weights)

```{r sample weighted uncencorceed Head count ratio}
merged_dat %>% 
  summarise(
    mean_uncensored_H_d_Asst = weighted.mean(merged_dat$d_asst,
                                             w = weight),
    mean_uncensored_H_d_ckfl = weighted.mean(merged_dat$d_ckfl,
                                             w = weight),
    mean_uncensored_H_d_hsg = weighted.mean(merged_dat$d_hsg,
                                             w = weight),
    mean_uncensored_H_d_sani = weighted.mean(merged_dat$d_sani,
                                             w = weight),
    mean_uncensored_H_d_wtr = weighted.mean(merged_dat$d_wtr,
                                             w = weight),
    mean_uncensored_H_d_elct = weighted.mean(merged_dat$d_elct,
                                             w = weight),
    mean_uncensored_H_d_educ = weighted.mean(merged_dat$d_educ,
                                             w = weight),
    mean_uncensored_H_d_satt = weighted.mean(merged_dat$d_satt,
                                             w = weight),
    mean_uncensored_H_d_cm = weighted.mean(merged_dat$d_cm,
                                             w = weight),
  ) %>% 
  transpose() %>% 
  unlist() %>% 
...
Follwoing are the Weights for each dimension:

```{r info dimension weights}
tibble(Dimensions = c("Assets", "Cooking Fuel", "Housing", "Sanitation",
                      "Water", "Electricity", "Education", 
                      "School Attendance", "Child Mortality"),
       Dimensional_Weights = c(rep(1/18,6), 1/6, 1/6, 1/3)) %>% 
  knitr::kable()
```
...
### Creating a weighted deprivation Matrix

```{r}
merged_dat %<>%
  mutate(
    g0_w_d_asst = d_asst * w_asst,
    g0_w_d_ckfl = d_ckfl * w_ckfl,
    g0_w_d_hsg = d_hsg * w_hsg,
    g0_w_d_sani = d_sani * w_sani,
    g0_w_d_wtr = d_wtr * w_wtr,
    g0_w_d_elct = d_elct * w_elct,
    g0_w_d_educ = d_educ * w_edu,
    g0_w_d_satt = d_satt * w_satt,
    g0_w_d_cm = d_cm * w_cm,
  )
...
merged_dat %<>%
  mutate(
    c_vector = structure(rowSums(.[,c(332:340)],na.rm = T),
                         label = "Counting Vector")
  )

### Table c_vector

merged_dat %>% 
  tabyl(c_vector, show_na = T) %>% 
  gt()
```
...
merged_dat %<>%
  mutate(
    multid_poor_33 = (c_vector >= 1/3),
    multid_poor_50 = (c_vector >= .5),
    multid_poor_20 = (c_vector >= .2)
  )
...
```{r censored Counting vector}
merged_dat %<>%
  mutate(
    cens_c_vector_33 = ifelse(multid_poor_33 == TRUE, c_vector,0),
    cens_c_vector_50 = ifelse(multid_poor_50 == TRUE, c_vector,0),
    cens_c_vector_20 = ifelse(multid_poor_20 == TRUE, c_vector,0)
  )
```

### M, H and A for all cut offs (1/3, .20, .50)

Comments from STATA code:

_By sumarizing (obtaining the mean) of the identification vector, the individual deprivation share,_ 

_and the individual censored c vector at any level of k we will obtain the Multidimensional Headcount_  

_Ratio (H), the Intensity of Poverty among the Poor (A), and the Adjusted Headcount Ratio (M0), respectively._

#### At k = 1/3

__Headcount/Incidence of Multidimensional Poverty __

```{r H_33}
weighted.mean(merged_dat$multid_poor_33, merged_dat$weight) 
```

At k = 1/3 , `r weighted.mean(merged_dat$multid_poor_33, merged_dat$weight)*100` % of the peopel are poor (multidimensionally deprived).

__Intensity__

```{r A_33}
merged_dat %>% 
  filter(multid_poor_33 == 1) %>% 
  summarise(
    Intensity = weighted.mean(cens_c_vector_33,weight)
  )

```

At k = 1/3, the average Intensity of poverty is `r weighted.mean(merged_dat$cens_c_vector_33[merged_dat$multid_poor_33 == 1], merged_dat$weight[merged_dat$multid_poor_33 == 1])*100`

__MPI__

M = A * I 

can also be calculated using the mean of the cens_c_vector at k = 1/3

```{r saving M_33}
merged_dat %>% 
  summarise(
    M = weighted.mean(cens_c_vector_33,weight)
  ) -> M_33
...
Now using the product of H and A

```{r M_33_product}
 weighted.mean(merged_dat$multid_poor_33, merged_dat$weight) * weighted.mean(merged_dat$cens_c_vector_33[merged_dat$multid_poor_33 == 1] , merged_dat$weight[merged_dat$multid_poor_33 == 1])

```

__Censored Deprivation Matrix__


Since here we would require the same operation to be performed over many columns, I would like to intriduce the concept for writing a small function in R as well as using it with the across function of the dplyr package. This technique could have been employed sooner in this document but I wanted the reader to get familiar with the basics before I jump into more advanced things.

Keep in mind the following stata code, we will wite an equivalent R code.

_foreach var in hh_d_school hh_d_nutri hh_d_water hh_d_assets {_ 
 
 _gen g0_ _`k'_ _`var' = `var'_  
 _replace g0_ _`k'_ _`var' = 0 if multid_poor_ _`k'==0  }_


First we write the function that uses the performs the conditional expression.

```{r funciton to create censored deprivation matrix}

create_censored_depreivation_mat <- function(x,y){
  ifelse(y == 0,0,x)
}

### Here the x argument will be the columns that show if a person is deprived in a certain dimension or not. In this case thee are named as d_Asst, d_ckfl etc.

### The y argument is the one that show if a person is poor (multidimensionally) at a given k. In this case it this coulmn can be multid_poor_33, multid_poor_20 etc.

```


```{r censored deprivation matrix k33}
merged_dat %>% 
  mutate(
    across(dplyr::starts_with("d_"), create_censored_depreivation_mat,
           multid_poor_33, .names = "g0_33_{col}")
  ) %>% 
  relocate(dplyr::starts_with("g0_33_")) %>% head()
```

```{r saving censored mat to merged data}
merged_dat %<>% 
  mutate(
    across(dplyr::starts_with("d_"), create_censored_depreivation_mat,
           multid_poor_33, .names = "g0_33_{col}")
...
__Censored Headcount Ratios for Indicators__


```{r censored headcount ratios for indicators}
merged_dat %>% 
  summarise(
    across(dplyr::starts_with("g0_33_"), 
           weighted.mean,weight, .names = "cen_H_{col}")
  ) %>% 
  transpose() %>% 
  unlist() %>% 
  as.data.frame() %>% 
...
__Dimensional Breakdown: Percentage contribution__


```{r Dimensional Breakdown_percentage constribution}
merged_dat %>% 
  summarise(
    across(dplyr::starts_with("g0_33_"), 
           weighted.mean,weight, .names = "cen_H_{col}")
  ) %>% 
  pivot_longer(cols = cen_H_g0_33_d_cm:cen_H_g0_33_d_asst,
               names_to = "Dimension",
               values_to = "Headcount_ratio_for_indicators_atk33" ) %>% 
  mutate(
    Dimension = str_extract(Dimension, "d_.*"),
    dimensional_weights = c(1/3, NA, 1/6, 1/6, 1/18, 1/18, 1/18, 1/18, 1/18, 1/18),
    percentage_contribution_by_indicator = 
      (Headcount_ratio_for_indicators_atk33 * dimensional_weights)/M_33
  ) %>% 
  knitr::kable() %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  kableExtra::row_spec(0,bold = T,color = "#ffffff" ,background = "#009688") %>%  
  kableExtra::scroll_box(width = "100%", height = "100%")
```

__Subgroup Decomposition__

_Regional Headcount/incidence of Multidimensional Poverty_

```{r regional H_33, message=FALSE}
merged_dat %>% 
  group_by(region) %>% 
  summarise(
    H_region_33 = weighted.mean(multid_poor_33, weight)
  ) %>% 
  knitr::kable() %>% 
  kableExtra::kable_styling()
```

_Regional Intensity of Multidimensional Poverty_


```{r regional A_33, message=FALSE}
merged_dat %>% 
  filter(multid_poor_33 == 1) %>% 
  group_by(region) %>% 
  summarise(
    Intensity_Region_33 = weighted.mean(cens_c_vector_33,weight)
  )%>%
  knitr::kable()%>%
  kableExtra::kable_styling()
```
...
```{r regional MPI_33, message=FALSE}
merged_dat %>% 
  group_by(region) %>% 
  summarise(
    M_regional_33 = weighted.mean(cens_c_vector_33,weight)
  ) %>%
  knitr::kable()%>%
  kableExtra::kable_styling()
```

_Regional Censored Headcount Ratios for Indicators_

```{r regional censored headcount for indicators, message=FALSE}
merged_dat %>% 
  group_by(region) %>% 
  summarise(
    across(dplyr::starts_with("g0_33_"), 
...
_Regional Contributuion to Poverty_

```{r regional contribution to poverty, message=FALSE}
merged_dat %>% 
  group_by(region) %>% 
  summarise(
    pop_share = sum(weight)/sum(merged_dat$weight),
    M_regional_33 = weighted.mean(cens_c_vector_33, weight),
    Regional_Contribution = pop_share * M_regional_33/M_33
  ) %>%
  knitr::kable()%>%
  kableExtra::kable_styling()
